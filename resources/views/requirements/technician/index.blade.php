...
@section('content')
    ...
    <x-ticket_elements>
        <div>
            ...
            <label class="w-4 h-4 text-gray-500" >/</label>
            <a href="{{route('technician.requirements.index')}}" class="@if(!isset($old))text-black @else text-gray-500  @endif  hover:text-blue-500 font-bold w-auto  m-1">
                Požadavky
            </a>
            <label class="w-4 h-4 text-gray-500" >|</label>
{{--            {{route('technician.requirements.index.old')}}--}}
            <a href="{{route('technician.requirements.index.old')}}" class="@if(isset($old))text-black @else text-gray-500  @endif  hover:text-blue-500 font-bold w-auto  m-1">
                Archiv požadavků
            </a>
        </div>

        <form action="{{route('technician.requirements.index')}}">
            <div class="flex items-center justify-center p-6">
                ...
                <a href="{{route('technician.requirements.index')}}" class="bg-yellow-500 text-center text-white  hover:bg-green-500 px-5 py-2.5 rounded-lg font-bold w-auto flex items-center justify-center">
                    Obnovit
                </a>
            </div>
        </form>
