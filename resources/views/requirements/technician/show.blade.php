...
@section('content')
    <x-ticket_elements>
        <div class="pb-6">
            ...
            <label class="w-4 h-4 text-gray-500" >/</label>
            <a href="{{route('technician.requirements.show', $requirement->id)}}" class="text-black  hover:text-blue-500 font-bold w-auto  m-1">
                Informace o požadavku
            </a>
            <label class="w-4 h-4 text-gray-500" >|</label>
            <a href="{{route('technician.requirements.index.old')}}" class="text-gray-500 hover:text-blue-500 font-bold w-auto  m-1">
                Archiv požadavků
            </a>
        </div>
        ...
        <div class="mb-5  break-after-left pl-10 text-3xl rounded-lg ">
            <div>{{$requirement->price}}</div>
                ...
                <a href="{{route('technician.requirements.edit', $requirement->id)}}" class="bg-blue-700 text-center text-white  hover:bg-blue-500 px-5 py-2.5 rounded-lg font-bold  items-center justify-center">
                    Provést změny
                </a>
                </div>
        ...
        @foreach($comments as $comment)
            <div class="mr-auto my-3  w-fit p-6 bg-white border border-black rounded-lg">
                <label class="w-fit break-all border-b border-black text-xl mb-2">@if($comment->user_id === \Illuminate\Support\Facades\Auth::id()) Vy @else Správce města @endif | {{$comment->created_at}}</label>
        ...
        @endforeach
            ...
            <x-form_elements.form action="{{route('technician.requirements.comment', $requirement->id)}}">
                <div class="mx-5">
                    <div class="mb-2">
